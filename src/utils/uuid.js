 ...
 */
  UUID.generate = function() {
    var rand = UUID._getRandomInt; var hex = UUID._hexAligner
 ...
 */
  UUID._hexAligner = function(num, length) {
    var str = num.toString(16); var i = length - str.length; var z = '0'
 ...
 */
   ...
   */
    ...
    var crypto = null; var cryptoPRNG = mathPRNG
    if (typeof window !== 'undefined' && (crypto = window.crypto || window.msCrypto)) {
      ...
      // nodejs
        cryptoPRNG = function(x) {
          if (x < 0 || x > 53) { return NaN }
          var buf = crypto.randomBytes(x > 32 ? 8 : 4); var n = buf.readUInt32BE(0)
 ...
 */
  UUID.parse = function(strId) {
    var r; var p = /^\s*(urn:uuid:|\{)?([0-9a-f]{8})-([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{2})([0-9a-f]{2})-([0-9a-f]{12})(\})?\s*$/i
    if (r = p.exec(strId)) {
      var l = r[1] || ''; var t = r[8] || ''
      if (((l + t) === '') ||
        (l === '{' && t === '}') ||
 ...
 */
  UUID.prototype._init = function() {
    var names = UUID.FIELD_NAMES; var sizes = UUID.FIELD_SIZES
    var bin = UUID._binAligner; var hex = UUID._hexAligner
 ...
 */
  UUID._binAligner = function(num, length) {
    var str = num.toString(2); var i = length - str.length; var z = '0'
    for (; i > 0; i >>>= 1, z += z) { if (i & 1) { str = z + str } }
    return str
  }
 ...
 */
  UUID.genV1 = function() {
    ...
    var now = new Date().getTime(); var st = UUID._state
    if (now != st.timestamp) {
